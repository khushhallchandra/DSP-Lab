...
Int16 direct_form_1(Int16 i,Int16 input){	
    ...
    acc = x_IIR[i]*num_coeff[0]+x_IIR[(i+1)%3]*num_coeff[2]+x_IIR[(i+2)%3]*num_coeff[1];
    acc -= (y_IIR[(i+1)%3]*den_coeff[2] + y_IIR[(i+2)%3]*den_coeff[1]);
    acc = acc/100;
...
int main( void ){
    Int16 i,input,output,j=0,k=0;	/* Initialize BSL */
    SYS_EXBUSSEL = 0x6100;
    USBSTK5515_init( );
